      ...
      `}>
        ...
        <div className="p-5 space-y-3">
            ...
            <button 
                ...
                className="w-full flex items-center justify-center gap-3 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-amber-500 py-3 px-4 rounded border border-slate-800 hover:border-slate-700 transition-all group"
            >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>
                <span className="font-serif tracking-wide text-sm font-semibold">SAVE CHAT</span>
            </button>

            <div className="grid grid-cols-2 gap-2">
                <button 
                    onClick={() => setIsExamRepoOpen(true)}
                    className="w-full flex flex-col items-center justify-center gap-1 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-slate-200 py-3 px-2 rounded border border-slate-800 hover:border-slate-700 transition-all group"
                >
                    <svg className="w-5 h-5 group-hover:text-amber-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 14l9-5-9-5-9 5 9 5z" /><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" /></svg>
                    <span className="font-serif tracking-wide text-[10px] font-semibold uppercase">AIBE Papers</span>
                </button>
                <button 
                    onClick={() => setIsMockTestOpen(true)}
                    className="w-full flex flex-col items-center justify-center gap-1 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-amber-500 py-3 px-2 rounded border border-slate-800 hover:border-slate-700 transition-all group"
                >
                    <svg className="w-5 h-5 group-hover:text-amber-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
                    <span className="font-serif tracking-wide text-[10px] font-semibold uppercase">Mock Test</span>
                </button>
            </div>
        </div>
              ...
              savedSessions.map((session) => (
                <div key={session.id} className="group relative">
                    <button 
                        ...
                        className="w-full text-left p-3 rounded-lg bg-slate-900/50 hover:bg-slate-900 transition-all border border-transparent hover:border-slate-800 flex flex-col gap-1 group-hover:shadow-md"
                    >
                        <div className="flex items-center gap-2">
                            <span className="text-amber-700 group-hover:text-amber-600 transition-colors text-lg leading-none">¶</span>
                            <span className="text-sm font-medium text-slate-400 group-hover:text-slate-200 truncate font-serif">{session.title}</span>
                        </div>
                        <span className="text-[10px] text-slate-600 pl-5 uppercase tracking-wider">
